...
#ifdef DEBUG
      debugI("microswitch CHANGE @ %d",local_angle);
#endif
      delay(5);
      microSwitchState = digitalRead(microSwitchPin);
...
#ifdef DEBUG
      debugI("About to start the pause");
#endif
            pause = true;
...
#ifdef DEBUG
      debugI("Playing at master's request");
#endif
          servoPID.SetMode(AUTOMATIC);
        }
        else {
...
#ifdef DEBUG
      debugI("Loop waiting for master to cross 0");
#endif
...
#ifdef DEBUG
      debugI("Starting beginning_procedure");
#endif
...
#ifdef DEBUG
      debugI("We reached the beginning");
#endif
        reset_offset();
...
#ifdef DEBUG
      debugI("exiting beginning_procedure");
#endif
